<?php 
	...
	$sql_get_friends = "SELECT * FROM friends WHERE (user1 = '{$logged_user}' AND level = '1') OR (user2 = '{$logged_user}' AND level = '1') ORDER BY RAND() LIMIT 8";
	...
	<div class="friends-list">
		...
		if ($number_of_friends > 0) {
			while ($row_get_friends = $query_get_friends->fetch_assoc()) {
				if ($row_get_friends["user2"] != $logged_user) {
				...
				} else if ($row_get_friends["user1"] != $logged_user) {
